...
export const ShiftReportsPage: React.FC<ShiftReportsPageProps> = ({ reports, workOrders, currentUserEmail, onAdd, onDelete }) => {
  ...
  const handleCrossCheckAnalysis = async () => {
      ...
      try {
          ...
          const response = await ai.models.generateContent({ model: "gemini-2.0-flash", contents: contents, config: { responseMimeType: "application/json", systemInstruction: systemInstruction } });
